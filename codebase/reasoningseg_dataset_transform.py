...
def main():
    dataset_root = "/training/zilun/vlm-r1seg/dataset"
    dataset_name = "reasoningseg"
    data_dir = os.path.join(dataset_root, dataset_name, "test")
    mask_dir = os.path.join(dataset_root, dataset_name, "mask_img_test")
    save_json_path = os.path.join(dataset_root, "reasonseg_test.json")
    os.makedirs(data_dir, exist_ok=True)
    os.makedirs(mask_dir, exist_ok=True)
    dump_masks(data_dir, mask_dir)
    print("All masks dumped")
    all_json = os.listdir(data_dir)
    all_json = [f for f in all_json if f.endswith("json")]
    all_fname = [f.split(".")[0] for f in all_json]
    json_list = []
